...
Window {
    ...
    property string windowIcon: HcApp.windowIcon
    property int launchMode: HcWindowType.Standard
    property var argument:({})
    property var background : com_background
    property bool fixSize: false
    property Component loadingItem: com_loading
    property bool fitsAppBarWindows: false
    property var tintOpacity: HcTheme.dark ? 0.80 : 0.75
    ...
    property color backgroundColor: {
        if(frameless.effective && active){
            ...
            if(frameless.effect==="dwm-blur"){
                backcolor = HcTools.withOpacity(HcTheme.windowActiveBackgroundColor, window.tintOpacity)
            }else{
        ...
        if(active){
            return HcTheme.windowActiveBackgroundColor
        }
        return HcTheme.windowBackgroundColor
    }
    ...
    property color resizeBorderColor: {
        if(window.active){
            return HcTheme.dark ? Qt.rgba(51/255,51/255,51/255,1) : Qt.rgba(110/255,110/255,110/255,1)
        }
        return HcTheme.dark ? Qt.rgba(61/255,61/255,61/255,1) : Qt.rgba(167/255,167/255,167/255,1)
    }
    property int resizeBorderWidth: 1
    property var closeListener: function(event){
        if(autoDestroy){
            HcRouter.removeWindow(window)
        }else{
    ...
    id: window
    color: HcTools.isSoftware() ? window.backgroundColor : "transparent"
    Component.onCompleted: {
        HcRouter.addWindow(window)
        useSystemAppBar = HcApp.useSystemAppBar
    ...
    HcFrameless{
        ...
        disabled: HcApp.useSystemAppBar
        isDarkMode: HcTheme.dark
        useSystemEffect: !HcTheme.blurBehindWindowEnabled
        Component.onCompleted: {
        ...
        onEffectiveChanged: {
            if(effective){
                HcTheme.blurBehindWindowEnabled = false
            }
        }
    }
    Component{
        ...
        Item{
            ...
            Image{
                ...
                Component.onCompleted: {
                    ...
                    source = HcTools.getUrlByFilePath(HcTheme.desktopImagePath)
                }
                Connections{
                    target: window
                ...
                function updateLayout(){
                    var geometry = HcTools.desktopAvailableGeometry(window)
                ...
                Connections{
                    ...
                    function onBlurBehindWindowEnabledChanged(){
                        if(HcTheme.blurBehindWindowEnabled){
                            img_back.source = HcTools.getUrlByFilePath(HcTheme.desktopImagePath)
                        }else{
                            img_back.source = ""
                        }
                    }
            ...
            HcAcrylic{
                ...
                visible: window.active && HcTheme.blurBehindWindowEnabled
                tintColor: HcTheme.dark ? Qt.rgba(0, 0, 0, 1)  : Qt.rgba(1, 1, 1, 1)
    ...
    Item{
        ...
        HcLoader{
            ...
            anchors.fill: parent
            sourceComponent: {
                if(window.useSystemAppBar || HcTools.isWin() || window.visibility === Window.Maximized || window.visibility === Window.FullScreen){
                    return undefined
                }
                return com_border
            }
    ...
    function moveWindowToDesktopCenter(){
        var availableGeometry = HcTools.desktopAvailableGeometry(window)
