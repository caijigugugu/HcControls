...
HcFrameless::HcFrameless(QQuickItem *parent) : QQuickItem{parent} {
    _fixSize = false;
    _appbar = nullptr;
    ...
    _effect = "normal";
    _effective = false;
    _isWindows11OrGreater = HcTools::getInstance()->isWindows11OrGreater();
}

HcFrameless::~HcFrameless() = default;

[[maybe_unused]] void HcFrameless::onDestruction() {
    QGuiApplication::instance()->removeNativeEventFilter(this);
}

void HcFrameless::componentComplete() {
#ifdef Q_OS_WIN
    ...
    connect(this, &HcFrameless::effectChanged, this, [hwnd, this] {
    ...
    connect(this, &HcFrameless::useSystemEffectChanged, this, [this] {
        if (!_useSystemEffect) {
            effect("normal");
        }
    });
    connect(this, &HcFrameless::isDarkModeChanged, this, [hwnd, this] {
...
#  endif
    ...
    connect(this, &HcFrameless::topmostChanged, this, [this] {
        _setWindowTopmost(topmost());
    });
    _setWindowTopmost(topmost());
}

[[maybe_unused]] bool HcFrameless::nativeEventFilter(const QByteArray &eventType, void *message, QT_NATIVE_EVENT_RESULT_TYPE *result) {
#ifdef Q_OS_WIN
...
bool HcFrameless::_isMaximized() {
    return window()->visibility() == QWindow::Maximized;
}

bool HcFrameless::_isFullScreen() {
    return window()->visibility() == QWindow::FullScreen;
}

void HcFrameless::_showSystemMenu(QPoint point) {
#ifdef Q_OS_WIN
...
bool HcFrameless::_hitMaximizeButton() {
...
void HcFrameless::_setMaximizePressed(bool val) {
    if (_maximizeButton) {
        _maximizeButton->setProperty("down", val);
    }
}

void HcFrameless::_setMaximizeHovered(bool val) {
    if (_maximizeButton) {
        _maximizeButton->setProperty("hover", val);
    }
}

void HcFrameless::_updateCursor(int edges) {
    switch (edges) {
        case 0:
...
[[maybe_unused]] void HcFrameless::showFullScreen() {
    window()->showFullScreen();
}

void HcFrameless::showMaximized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 3);
#else
...
[[maybe_unused]] void HcFrameless::showMinimized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 2);
#else
...
void HcFrameless::showNormal() {
    window()->setVisibility(QQuickWindow::Windowed);
}

void HcFrameless::setHitTestVisible(QQuickItem *val) {
    if (!_hitTestList.contains(val)) {
        _hitTestList.append(val);
    }
}

void HcFrameless::_setWindowTopmost(bool topmost) {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    if (topmost) {
...
bool HcFrameless::eventFilter(QObject *obj, QEvent *ev) {
#ifndef Q_OS_WIN
    switch (ev->type()) {
